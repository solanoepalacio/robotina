...
**Domain:** Python AI agent with task queue, Postgres persistence, and LLM observability
**Researched:** 2026-03-25
**Confidence:** MEDIUM — stack is fully specified by the project; version numbers sourced from training data (knowledge cutoff Aug 2025). Verify with `uv add <package>` to get latest at install time.
...
uv add langchain-anthropic>=0.3 langchain-openai>=0.2 langchain-ollama>=0.2
uv add rq>=2.5 redis>=5.0
uv add sqlalchemy>=2.0 alembic>=1.13 psycopg2-binary
uv add pydantic>=2.7
uv add langwatch opentelemetry-sdk>=1.25 opentelemetry-api>=1.25
uv add python-telegram-bot>=21
uv add tavily-python>=0.3
uv add fastapi>=0.115 uvicorn>=0.30 httpx>=0.27
...
---
*Stack research for: Robotina — Python AI home assistant agent*
*Researched: 2026-03-25*
