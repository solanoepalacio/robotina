...
- [x] **ROBOT-01**: `handle-incoming-message` task type is handled by the Robotina agent
...
- [x] **ROBOT-04**: Robotina agent has access to `start-workflow` tool (initiate multi-step workflows)
...
- [x] **RLOAD-07**: The `recipe-load` agent binds `response_format=RecipeLoadOutput`; the workflow runner reads `structured_response` and persists it as the step artifact.
...
- [x] **DASH-04**: `src/robotina/dashboard/` module exists with a FastAPI app and a `dashboard = "robotina.dashboard:main"` entry in `pyproject.toml [project.scripts]`; `uv run dashboard` starts uvicorn on `DASHBOARD_PORT` (default 8001)
- [x] **DASH-05**: `GET /` returns 200 HTML listing the latest 50 `WorkflowRun` rows ordered by `created_at DESC`; empty state renders cleanly with zero runs
...
| ROBOT-01 | Phase 7 | Complete |
| ROBOT-02 | Phase 7 | Complete |
| ROBOT-03 | Phase 7 | Complete |
| ROBOT-04 | Phase 7 | Complete |
